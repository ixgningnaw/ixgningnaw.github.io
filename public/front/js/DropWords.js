...
function DropWords() {
    ...
    // 物理
    let drop_size = 18; // 接受创建和更新
    let drop_mass = 1;  // 接受创建和更新
    ...
    this.initializeText = function(in_text, in_gravity, in_wind) {
        ...
        origin_x = Math.floor(Math.random() * (max_x - min_y + 1)) + min_x;
    ...
    this.getStartPosition = function(in_id, in_y) {
        let m = (linear_function[in_id][3] - linear_function[in_id][1]) / (linear_function[in_id][2] - linear_function[in_id][0]);
        let b = linear_function[in_id][3] - (m * linear_function[in_id][2]);
        let start_x = (in_y - b) / m;
    ...
    this.getStartSpeed = function(current_id) {
        ...
        let current_speed = (origin_y - text_y[current_id] - (((gravity * iterations) + gravity) * (iterations / 2))) / iterations;
